DROP FUNCTION IF EXISTS ana.wow_time1(startDate TIMESTAMP, endDate TIMESTAMP , inputusername VARCHAR , inputserserialno VARCHAR );
CREATE FUNCTION ana.wow_time1(startDate TIMESTAMP, endDate TIMESTAMP ,inputusername VARCHAR='',  inputserserialno VARCHAR  ='')
RETURNS TABLE 
    (
        username VARCHAR,
        serialno VARCHAR,
...
BEGIN
    ...
    (
        ...
        FROM game AS ga
        WHERE startdatetime::time BETWEEN ('05:00'- interval '8 hour')::time  AND ('13:00'- interval '8 hour')::time --台灣時間減八小時為資料庫伺服器時間
    ),
    result AS
    (
...
select * from ana.wow_time1 (timestamp '2019-11-11','2019-11-17');
